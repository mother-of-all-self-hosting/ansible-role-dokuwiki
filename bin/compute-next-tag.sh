...
#
# Tags look like `v<DokuWiki version>-<release>`:
#
# - if defaults/main.yml points at a DokuWiki version that has never been
#   released, the release counter restarts at 0 (`v2025-05-14b-0`)
# - otherwise the counter is incremented (`v2025-05-14b-1`), but only if something
...
version="$(sed -nE 's|^dokuwiki_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the DokuWiki version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `2025-05-14b`), but the tags
# do (`v2025-05-14b-0`). Stripping any `v` before prepending one keeps this correct
